...
function cb(error, response, html) {
    ...
    // element select 
    let matchcard = cheerioSelector(".match-info-link-FIXTURES");
    console.log(matchcard.length);
    gCount = matchcard.length;
    for (let i = 0; i < matchcard.length; i++) {
        // sublock anchors
        let link = cheerioSelector(matchcard[i]).attr("href");
...
function cbsingle(err, response, html) {
    ...
    // team Name [MI,DC]
    let bothInningsTeamName = chSelector(".Collapsible .col");
